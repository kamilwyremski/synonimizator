...
export class AppPage {
  ...
  setInputTitle(value) {
    const input =  element(by.id('title_input'))
  ...
  setInputKeywords(value) {
    const input = element(by.id('keywords_input'))
    input.sendKeys(value);
    input.sendKeys(Key.TAB);
  }

  async getInputKeywords() {
    const value = await element(by.id('keywords_output')).getAttribute('value')
  ...
  setInputDesription(value) {
    browser.executeScript(`
      ...
      button.click();
    `);
    const input = element(by.id('description_input'))
    input.sendKeys(value);
    input.sendKeys(Key.TAB);
  }
